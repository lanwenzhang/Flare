...
namespace flare::vk {
	...
	// Load the create debug message function
	...
	Instance::Instance(bool enableValidationLayer){
		...
		// 2.2 Enable validation layers
		if (mEnableValidationLayer) {
			instCreateInfo.enabledLayerCount = static_cast<uint32_t>(validationLayers.size());
			instCreateInfo.ppEnabledLayerNames = validationLayers.data();
		}

		// 3 Create instance handle
	...
	bool Instance::checkValidationLayerSupport() {
		...
		// 1 Get the number of all the validation layers
		uint32_t layerCount = 0;
		vkEnumerateInstanceLayerProperties(&layerCount, nullptr);

		// 2 Get the data of all the validation layers
		std::vector<VkLayerProperties> availableLayers(layerCount);
		vkEnumerateInstanceLayerProperties(&layerCount, availableLayers.data());

		// 3 Check if the validation layer that we request is provided
